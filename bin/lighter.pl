...
my $screen_script="/sys/class/backlight/acpi_video0/brightness";
...
my $keyb_script="/sys/devices/platform/applesmc.768/leds/smc::kbd_backlight/brightness";
...
sub keyb_now {
    $keyb_now=trim(`cat '/sys/devices/platform/applesmc.768/leds/smc::kbd_backlight/brightness'`);
    debug_msg( $keyb_now."<-- keyb now\n" );
}

sub screen_now {
    $screen_now=trim(`cat '/sys/class/backlight/acpi_video0/brightness'`);
